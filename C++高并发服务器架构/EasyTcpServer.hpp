...
public:
	...
	//客户端消息事件
	virtual void OnNetMsg(ClientSocket* pClient, netmsg_DataHeader* header) = 0;
	//recv事件
...
public:
	...
	bool OnRun()
	{
		if (isRun())
		{
			...
			if (ret < 0)
			{
				CELLLog::Info("Accept Select任务结束。\n");
				Close();
				return false;
			}
			//判断描述符（socket）是否在集合中
